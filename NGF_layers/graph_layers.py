...
def neighbour_lookup(atoms, edges, maskvalue=0, include_self=False):
    ...
    flattened_edges = K.reshape(tf.add(tf.cast(masked_edges,tf.int32), tf.cast(offset,tf.int32)), (batch_n, -1))
...
class NeuralGraphHidden(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for degree in range(max_degree):
            ...
            with tf.name_scope("dense_degree_"+str(degree)):
              ...
              self.train_weights += inner_3D_layer.trainable_weights
    ...
    def call(self, input, mask=None):
        ...
        num_samples = K.int_shape(atoms)[0]
        max_atoms = K.int_shape(atoms)[1]
        num_atom_features = K.int_shape(atoms)[-1]
        num_bond_features = K.int_shape(bonds)[-1]

        #num_samples = atoms._shape_val[0]
        #max_atoms = atoms._shape_val[1]
        #num_atom_features = atoms._shape_val[-1]
        #num_bond_features = bonds._shape_val[-1]
...
class NeuralGraphOutput(layers.Layer):
    ...
    def __init__(self, inner_layer_arg, activ, bias , init, **kwargs):
        # Initialise based on one of the three initialisation methods
        super(NeuralGraphOutput, self).__init__()
